...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-secret-envelope-consumer-' . bin2hex(random_bytes(6));
...
consumerRun(
    [
        ...
        '--format=zip',
        '--dir=' . $distDirectory,
        '--file=kumwe-secret-envelope',
    ],
    $workspace,
);
$archive = $distDirectory . '/kumwe-secret-envelope.zip';
if (!is_file($archive)) {
...
$consumerMetadata = [
    ...
    'license' => 'proprietary',
    'require' => ['kumwe/secret-envelope' => $release],
    'repositories' => [
...
$installed = $consumer . '/vendor/kumwe/secret-envelope';
...
consumerRun([PHP_BINARY, $installed . '/examples/round-trip.php', $autoload], $workspace);
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\Secret\\Tests\\')
    ) {
...
// A second stage proves the optional host integration only after runtime-only consumption passed.
consumerRun([
    'composer', '--working-dir=' . $consumer, 'require', 'laminas/laminas-servicemanager:^4.5',
    '--no-interaction', '--no-progress', '--update-no-dev', '--no-plugins', '--no-scripts', '--classmap-authoritative',
], $workspace);
