...
The portable behavior, boundary and conformance suites belong to kumwe/secret-envelope.
...
- Host key custody, HKDF labels, rotation/restore, authorized record access, cross-record binding and audit.
...
The complete package gate includes release parsing, integrity and publication/setup regression fixtures.
Normal publication and independent verification are distinct from optional administrator hardening.
Existing tags and releases are never replaced. See [releasing](releasing.md) for exact source identity,
publication retries, optional hardening and consumer verification requirements.
